...
| `PUBLIC_APP_ISSUE_URL`                                   | runtime                               | URL for reporting issues, shown on the `/settings` feedback link.                                                                                                                                                                                                                                                                                                                                                                                                            |
...
`/privacy` renders `PRIVACY.md` from the repo root (`src/routes/privacy/+page.server.ts` reads it
with a server-side `load()`). Unlike the vars in the table above, this is **build time only** —
`Dockerfile` copies `PRIVACY.md` into the image (`COPY --chown=1000 PRIVACY.md /app/PRIVACY.md`),
and the running app reads that baked-in copy, not the file in your working repo. Editing
`PRIVACY.md` and pushing the commit does nothing to an already-running container: the image has
to be rebuilt and redeployed for the change to reach `/privacy`.
